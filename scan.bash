...
_arxguard_scan(){
 ...
 # Remote content piped into a shell is an execution boundary: block it rather than
 # merely warn. This covers direct interpreters and argument-bearing forms such as
 # `sh -c`, while preserving the separate warnings for downloads that are not piped.
 [[ "$lc" =~ (curl|wget|fetch)[[:space:]].*https?://.*\|[[:space:]]*(sudo[[:space:]]+)?(ba|z|da|c|k)?sh([[:space:]]|$) ]]&&_f 2 "[CRITICAL] remote script piped into shell"
